...
test('should create a regex that match against any node module set aside given ones', () => {
  const regex = createExcludeRegex(['oc-template-typescript-react-compiler/utils', 'underscore']);
  ...
  expect(regex.test('node_modules/oc-template-typescript-react-compiler/utils')).toBe(false);
